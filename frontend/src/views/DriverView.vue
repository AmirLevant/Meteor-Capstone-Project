            ...
            <div class="mb-4">
              ...
              <div v-if="availableRoutes.length > 0" class="mb-3">
                ...
                >
                  ...
                  <option 
                    ...
                    :value="route.id"
                  >
                    Route {{ route.id.split('_')[1] }} ({{ route.roads?.length || 0 }} roads)
                  </option>
                </select>
              </div>
              ...
              <div v-if="selectedRoute" class="mb-3 p-3 bg-blue-50 rounded-lg">
                <p class="text-sm text-blue-800">
                  <strong>Active Route:</strong> {{ selectedRoute.roads?.length || 0 }} road segments
                </p>
...
const displayRouteOnMap = (roads) => {
  ...
  roads.forEach((road, index) => {
    if (road.geometry && road.geometry.coordinates) {
      ...
      // Add road as a polyline with different colors based on road type
      const roadColors = {
        'primary': '#ff4444',
        'secondary': '#ff8844',
        'tertiary': '#ffaa44', 
      ...
      const color = roadColors[road.type] || roadColors[road.properties?.highway_type] || '#666666'
      ...
      polyline.bindPopup(`
        <div>
          ...
          Type: ${road.type || road.properties?.highway_type || 'Unknown'}<br>
          ${road.properties?.surface ? `Surface: ${road.properties.surface}` : ''}
        </div>
      `)
...
const toggleTracking = () => {
  ...
  if (isTracking.value) {
    ...
    console.log('Started tracking route with', selectedRoute.value.roads?.length, 'roads')
  } else {
    // Stop tracking
